...
public class MillionaireFinally {

	static int maximizeProfit(int[] a, int[] b, int m, int k) {
        int maxValue=m*k;
        int length=a.length;
        for(int i=0;i<length;i++) {
            int convValue=m*a[i]*b[i];
            maxValue=(maxValue>=convValue)?maxValue:convValue;
        }
        return maxValue;
    }

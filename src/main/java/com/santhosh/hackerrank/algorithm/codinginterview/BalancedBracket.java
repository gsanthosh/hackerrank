...
public class BalancedBracket {	
	static Set<Character> openSet=new HashSet<>();
    static Set<Character> closeSet=new HashSet<>();
    static Map<Character,Character> pairMap=new HashMap<Character,Character>();
    static{
        openSet.add('{');
        openSet.add('(');
        openSet.add('[');
    ...
    public static boolean isBalanced(String expression) {
        Deque<Character> charDeque=new LinkedList<Character>();
        ...
        for(int i=0;i<length;i++){
           Character currentChar=expressionArray[i];
            if(openSet.contains(currentChar)){
               charDeque.addFirst(currentChar);
            }else{
                if(charDeque.isEmpty()){
                    return false;
                }
                if(!charDeque.removeFirst().equals(pairMap.get(currentChar))){
                    return false;
                }
            }
        }
